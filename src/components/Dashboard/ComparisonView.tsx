...
import { IResume, IOptimizedResume } from '../../types';

type TabType = 'side-by-side' | 'diff-view';
...
const ComparisonView: React.FC = () => {
  ...
  // Load resume data
  useEffect(() => {
    if (!auth?.getResumeById || !id) return;
    
    const resumeData = auth.getResumeById(id);
    if (resumeData) {
      setResume(resumeData as IResume);
    }
  }, [id, auth]);
  ...
  const originalResume: IOptimizedResume = {
    name: "John Doe",
    ...
    keywordMatches: 85,
    improvements: [
      "Added key technologies from job description",
      "Quantified achievements with metrics",
      "Optimized experience section for ATS scanning",
